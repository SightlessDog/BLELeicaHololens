...
public class YoloV5Prediction
{
    ...
    public float ImageHeight = 640f;

    public IReadOnlyList<YoloV4Result> GetResults(float[] layer2, float[] layer3, float[] layer4, string[] categories, float scoreThres = 0.5f, float iouThres = 0.5f)
    {
        List<float[]> postProcesssedResults = new List<float[]>();
        int classesCount = categories.Length;
        var results = new[] { layer2, layer3, layer4 };
                ...
                for (int boxX = 0; boxX < outputSize; boxX++)
                {
                    for (int a = 0; a < anchorsCount; a++)
                    {
                        var offset = (boxY * outputSize * (classesCount + 5) * anchorsCount) +
                                     (boxX * (classesCount + 5) * anchorsCount) + a * (classesCount + 5);
                        var predBbox =
                            pred.Skip(offset).Take(classesCount + 5).Select(x => Sigmoid(x))
                                .ToArray(); // y = x[i].sigmoid()
                        ...
                        {
                            postProcesssedResults.Add(new float[]
                                { orgX1, orgY1, orgX2, orgY2, scoreMaxCat, scores.IndexOf(scoreMaxCat) });
                        }
                    }
                }
            }
